...
class SetupPoolDataCellCenters:
    def __init__(self, test_case_path):
        self.test_case_path = test_case_path

    def create_polygon(self, triangle):
        return Polygon(triangle)

    def calculate_overlap(self, refined_triangles, coarser_triangles):
        sources = []
        targets = []
        attributes = []

        # Create polygons and STRtrees for spatial indexing
        refined_polys = [self.create_polygon(triangle) for triangle in refined_triangles]
        coarser_polys = [self.create_polygon(triangle) for triangle in coarser_triangles]
    ...
    def getPoolData(self):
        ...
        with ProcessPoolExecutor() as executor:
            for i in range(self.nLevels - 1):
                refined_triangles = dataDictMeshes[i]["pointCoordinates"][dataDictMeshes[i]["cellPoints"]]
                coarser_triangles = dataDictMeshes[i + 1]["pointCoordinates"][dataDictMeshes[i + 1]["cellPoints"]]
                tab_pooling.append(executor.submit(self.calculate_overlap, coarser_triangles, refined_triangles))
                tab_unpooling.append(executor.submit(self.calculate_overlap, refined_triangles, coarser_triangles))
        ...
        file_path = f"{self.test_case_path}/dataDictPoolCellCenters.pkl"
    ...
    def getPoolData_direct(self):
        ...
        with ProcessPoolExecutor() as executor:
            for i in range(self.nLevels - 1):
                refined_triangles = dataDictMeshes[0]["pointCoordinates"][dataDictMeshes[0]["cellPoints"]]
                coarser_triangles = dataDictMeshes[i + 1]["pointCoordinates"][dataDictMeshes[i + 1]["cellPoints"]]
                tab_pooling.append(executor.submit(self.calculate_overlap, coarser_triangles, refined_triangles))
                tab_unpooling.append(executor.submit(self.calculate_overlap, refined_triangles, coarser_triangles))
        ...
        file_path = f"{self.test_case_path}/dataDictPoolCellCenters_direct.pkl"
        with open(file_path, 'wb') as file:
            pickle.dump(dataDictPool, file)
...
if __name__ == '__main__':
    ...
    pool = SetupPoolDataCellCenters(test_case_path)
    dataDictPool = pool.getPoolData()
    dataDictPool = pool.getPoolData_direct()
