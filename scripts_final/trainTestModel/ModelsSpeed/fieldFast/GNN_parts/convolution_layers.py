...
class CustomGraphConv(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr,graphData,weights_matrices):
        ...
        return self.pointCenter(x, edge_index, edge_attr,graphData,weights_matrices)
...
class CustomGraphConvPointPoint(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr,weights_matrices):
        ...
        return self.pointPoint(x, edge_index, edge_attr,weights_matrices)
...
class GNNpointPoint(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr,weights_matrices):
        ...
        return self.propagate(edge_index,weights_matrices=weights_matrices, x=x, edge_attr=edge_attr)
    ...
    def message(self, x_j, edge_attr,weights_matrices):
        ...
        return torch.matmul(weights_matrices, x_j.unsqueeze(-1)).squeeze(-1)
...
class GNNCenterPointCenterSimple(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, graphData,weights_matrices):
        ...
        # conv_result = self.propagate(edge_index,weights_matrices=weights_matrices, x=x, edge_attr=edge_attr, size=(graphData["inputSize"], graphData["outputSize"]))

        # conv_result = self.propagate(edge_index, x=x, edge_attr=edge_attr, size=(graphData["inputSize"], graphData["inputSize"]))

        return self.propagate(edge_index,weights_matrices=weights_matrices, x=x, edge_attr=edge_attr, size=(graphData["inputSize"], graphData["outputSize"]))
