...
class testNIterationsMultipleMeshesCFD:
    ...
    def runOneCase(self,i_case, ttab, results_dict,normalize_data=True,compute_diff_cfd=True):
        # print("dsfs",ttab)
        ...
        cellCenters = dataDictMeshes[0]["cellCenters"]

        on_off_tab = np.zeros(len(cellCenters))
        on_off_tab_block = np.zeros(len(cellCenters))

        for i in range(len(cellCenters)):
            if cellCenters[i,0]>7 and (cellCenters[i,1]>6.5 and cellCenters[i,1]<9.5):
                on_off_tab[i] = 1
            if cellCenters[i,0]>6.5 and cellCenters[i,0]<9.5 and (cellCenters[i,1]>6.5 and cellCenters[i,1]<9.5):
                on_off_tab_block[i] = 1

        on_off_tab = torch.tensor(on_off_tab).to(self.device).unsqueeze(0)
        on_off_tab_block = torch.tensor(on_off_tab_block).to(self.device).unsqueeze(0)

        print(on_off_tab.shape)
        print(on_off_tab_block.shape)

        print("fsaa",np.shape(cellCenters))
        ...
        testDataLoader = loadDataLoader(f"{self.datasetPath}/case_{i_case}",
                                        ...
                                        gradientBCInput=self.conf_field["modelParam"]["inputChannels"]["gradientBCInput"], doShuffling=False, normalize_data=normalize_data, compute_diff_cfd=compute_diff_cfd)
        ...
        with torch.no_grad():
            ...
            for p, pEqnD, pEqnFace, pEqnSource, inputCellArray, faceArray_incl_boundaries, BCArray, *factors in testDataLoader:
                ...
                inputFaceArray[:, dataDictMeshes[0]["boundaryFaces"], :2] = BCArray
                # print(self.device)
                inputFaceArray = torch.cat((faceArray_incl_boundaries, inputFaceArray), dim=2)
                # print("fsfsx")
                # print(BCArray.shape)
                # print(inputFaceArray.shape)
                ...
                # outP = torch.where(abs(outP) > 1.5, outP, 0)
                # print(outP.shape)
                # print(on_off_tab.shape)
                outP_copy = outP+0

                # outP = outP * on_off_tab.unsqueeze(-1)
                # outP = outP - torch.mean(outP)
                # diff = p.unsqueeze(-1).to('cuda') - outP
                # diff_outlier = torch.where(abs(diff)>3,diff,0)
                # diff = diff * on_off_tab_block.unsqueeze(-1)
                # outP = outP - torch.mean(outP)
                # outP = outP + diff

                # outP = outP + diff + diff_outlier
                # print(diff.shape)
                ...
                diff_outlier = torch.where(abs(diff)>4,diff,0)
                outP  += diff_outlier.to(self.device)
                diff = p.unsqueeze(-1) - outP.to("cpu")
                mean_diff = torch.mean(diff).item()
                outP += torch.mean(diff)

                # noise = torch.normal(mean=0.0, std=1/3, size=outP.shape).to(self.device)
                # outP += noise
                # L = 3
                # offset_fluctuation = 3 * torch.sin(torch.pi * torch.tensor(cellCenters[:,0]) / L) * torch.sin(torch.pi * (torch.tensor(cellCenters[:,1])-L/2) / L)
                # outP += offset_fluctuation.unsqueeze(0).unsqueeze(-1).to(self.device)

                # diff = p.unsqueeze(-1) - outP.to("cpu")
                # mean_diff = torch.mean(diff).item()
                # outP += torch.mean(diff)
                # outP += 0.9 * diff.to(self.device)
                ...
                # print(torch.max(abs(p)),torch.max(abs(outP_copy)))

                # if torch.max(abs(p))>8:
                # if iterations['p_zero_jacobi'] - iterations['p_model_jacobi'] >30:
                    ...
                    # print(outP_plot)
                    # outP_treshhold = torch.where(abs(outP_plot) > 1.5, 10, 0)
                    # pr

                    ax = fig.add_subplot(2, 2, 1)
                    plot_mesh_triangles(fig, ax, cellPoints, x_mesh, y_mesh, p[0], "ground_truth",zlim=zlim)
                    ax = fig.add_subplot(2, 2, 2)
                    plot_mesh_triangles(fig, ax, cellPoints, x_mesh, y_mesh, outP_plot[0, :, 0].cpu(), "prediction_raw",zlim=zlim)
                    ax = fig.add_subplot(2, 2, 3)
                    plot_mesh_triangles(fig, ax, cellPoints, x_mesh, y_mesh, outP_copy[0, :, 0].cpu()- p[0], "diff",zlim=zlim,centered_colorbar=True,apply_percentile=False)
                    ax = fig.add_subplot(2, 2, 4)
                    plot_mesh_triangles(fig, ax, cellPoints, x_mesh, y_mesh, outP_plot[0,:,0].cpu()- p[0], "diff corrected",centered_colorbar=True,apply_percentile=False)
                    # plot_mesh_triangles(fig, ax, cellPoints, x_mesh, y_mesh, on_off_tab, "source")
    ...
    def run(self,test_cases, ttab,compute_diff_cfd=True):
        ...
        for i_case in test_cases:
            results_dict = self.runOneCase(i_case, ttab, results_dict,compute_diff_cfd=compute_diff_cfd)
        ...
        print("")
        ratios_PCG = np.sum(results_dict["p_model_jacobi_iterations"]) / np.sum(results_dict["p_zero_jacobi_iterations"])
        ratios_GAMG = np.sum(results_dict["p_model_GAMG_iterations"]) / np.sum(results_dict["p_zero_GAMG_iterations"])
        print("PCG RATIOS: ", ratios_PCG)
        # print("PCG RATIOS PER CASE: ", np.mean(ratios_PCG,axis=1))
        print("")
        print("GAMG RATIOS: ", ratios_GAMG)
...
def autoTest(test_cases,ttab,config_files_field,config_files_mag,basePathDatasets,basePathLinearTest,datasetName,device,main_modeldir,save_name,iteration=None,compute_diff_cfd=True):
    ...
    for config_file_field in config_files_field:
        for config_file_mag in config_files_mag:
            ...
            results_dict = tester.run(test_cases, ttab,compute_diff_cfd=compute_diff_cfd)
...
if __name__ == '__main__':
    ...
    # Define model
    config_files_field = [f'config_files/foundParam_conv_4666_e3_100_small.yaml']
    ...
    # Save directory
    main_modeldir = f'/home/justinbrusche/modeldirs_cross_test/solverResults'
    save_name = f"perlin_on_step{step}_outlier_mean_good"
    ...
    print("ttab,", ttab)
    autoTest(test_cases,ttab,config_files_field,config_files_mag,basePathDatasets,basePathLinearTest,datasetName,device,main_modeldir,save_name,iteration,compute_diff_cfd=compute_diff_cfd)
