...
class CustomGraphConvStart(nn.Module):
    def __init__(self,in_channels, out_channels, GNNData,pointType=False):
        super().__init__()
        if pointType:
            self.centerPoint = GNNCenterPointCenterSimple(GNNData, in_channels, out_channels-1)
        else:
            self.centerPoint = GNNCenterPointCenterSimple(GNNData, in_channels, out_channels)

    def forward(self, x, edge_index, edge_attr, graphDataStep,pointTypes=None):
        x = self.centerPoint(x, edge_index, edge_attr, graphDataStep)
        if pointTypes is not None:
            shape_x = x.shape

            pointTypes = pointTypes.unsqueeze(0).unsqueeze(-1).expand(shape_x[0], shape_x[1], 1)
            x = torch.cat([x, pointTypes], dim=-1)
        return x

class CustomGraphConvEnd(nn.Module):
